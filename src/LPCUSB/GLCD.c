...
static __inline unsigned char ssp_skip (unsigned int byte)
{
	LPC_SSP1->DR = 0x1FF;
	while (LPC_SSP1->SR & (1 << 4));      /* Wait for transfer to finish        */
	return (LPC_SSP1->DR) & 0xFF;                /* Return received value              */
...
static __inline void skip (unsigned short c)
{
	LPC_GPIO0->FIOPIN &= ~(1 << 6);
	ssp_skip((c >> 8) | SSP_DATA);                       /* Write D8..D15                */
	ssp_skip((c & 0xFF) | SSP_DATA);
	LPC_GPIO0->FIOPIN |=  (1 << 6);
}
...
void GLCD_bitmap (unsigned int x, unsigned int y, unsigned int w, unsigned int h, unsigned int *bitmap)
{
	unsigned int   i;
...
void GLCD_bitmap2X (unsigned int x, unsigned int y, unsigned int w, unsigned int h, unsigned char *bitmap)
{
	unsigned int   i;
	unsigned int   len = w*h;
...
void GLCD_bitmap_Drawend (unsigned int x, unsigned int y, unsigned int w, unsigned int h, unsigned char *bitmap, unsigned int dex, unsigned int dey, unsigned int ox, unsigned int oy)
{
	unsigned int   i;
	unsigned int   len = w*h;
	unsigned short *bitmap_ptr = (unsigned short *)bitmap;
...
void GLCD_bitmap_transparent (unsigned int x, unsigned int y, unsigned int w, unsigned int h, unsigned int *bitmap )
{
	...
	unsigned int *bitmap_ptr = (unsigned int *)bitmap;
		...
		for(int b= 0+y; b<h+y; b++){
			for(int a = 0+x; a < w+x; a++){
			GLCD_setTextColor(*bitmap_ptr);
			if(*bitmap_ptr != 0XEE) {
				GLCD_putPixel(a,b);
...
void GLCD_bitmap_tScale(unsigned int x, unsigned int y, unsigned int w, unsigned int h, unsigned int *bitmap ,unsigned int scale)
{

	unsigned int *bitmap_ptr = (unsigned int *)bitmap;


unsigned int *backtop = (unsigned int *)bitmap;
		for(int b= 0; b<h; b++){
			backtop = bitmap_ptr;
			...
			if(*bitmap_ptr != 0XEE) {
				GLCD_putPixel(c+(scale*a) + x,(scale*(b))+t + y);
